...
def main():
    result = generate_does_one_assignment_overlap_with_other_assignment()
    print(sum(1 for does_contain in result if does_contain))


def generate_does_one_assignment_overlap_with_other_assignment() -> Generator[
    bool, Any, Any
]:
    for (start1, end1), (start2, end2) in generate_section_assignment_pairs():
        yield (
            (start1 >= start2 and start1 <= end2) or (end1 >= start2 and end1 <= end2)
        ) or (
